...
// ------------------  File Overview  ---------------------------------
// *   Part 1: Deprecated APIs                                        *
// *   Part 2: Compiler optimzations                                  *
// --------------------------------------------------------------------
...
[choco/getNbBacktracks(algo:GlobalSearchSolver) : integer
 -> when lim := some(l in algo.limits | l % BacktrackLimit) in 
         getNb(lim as BacktrackLimit)
    else 0]
[choco/getNbNodes(algo:GlobalSearchSolver) : integer
 -> when lim := some(l in algo.limits | l % NodeLimit) in 
         getNb(lim as NodeLimit)
    else 0]
[choco/getNbMillisec(algo:GlobalSearchSolver) : integer
 -> when lim := some(l in algo.limits | l % TimeLimit) in 
         time_read()
    else 0]

// ------- changes in v1.325

// became newTreeNode, endTreeNode
