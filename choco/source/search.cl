...
choco/newNode :: property(range = boolean)
choco/endNode :: property(range = void)
choco/reset :: property(range = void)
...
(interface(GlobalSearchLimit, newNode, endNode, reset))
...
[choco/makeSolutionFromCurrentState(a:Solver) : Solution
 -> let lvar := (if a.varsToStore a.varsToStore else a.problem.vars),
     ...
     (;sol.time := time_read(), // v1.013
      ...
      for i in (1 .. nbv)
        let v := lvar[i] in
           (if isInstantiated(v)  // bug in v1.324 !!!!
               sol.lval[i] := v.value),
      sol)]
...
[endTreeSearch(a:BranchAndBound) : void
 -> for lim in a.limits reset(lim,false), 
    ...
    else //[SVIEW] solve => no sol [~S]// a.limits
] 

[newLoop(a:OptimizeWithRestarts) : void -> initBounds(a), time_set()] // should we call a fullReset on limits ? (to reset cumulated counter?) 
