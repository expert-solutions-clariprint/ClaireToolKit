...
[choco/makeMinDomVarHeuristic() : MinDomain -> MinDomain()]
[choco/makeDomDegVarHeuristic() : DomDeg -> DomDeg()]
[choco/makeMaxDegVarHeuristic() : MaxDeg -> MaxDeg()]
[choco/makeStaticVarHeuristic(l:list[IntVar]) : StaticVarOrder
...
[choco/makeIncValIterator() : IncreasingDomain -> IncreasingDomain()]
[choco/makeDecValIterator() : DecreasingDomain -> DecreasingDomain()]

[choco/makeMinValSelector() : MinValHeuristic -> MinValHeuristic()]
[choco/makeMaxValSelector() : MaxValHeuristic -> MaxValHeuristic()]
[choco/makeMidValSelector() : MidValHeuristic -> MidValHeuristic()]
...
[choco/getNb(lim:CounterLimit) : integer -> lim.nb]
[choco/getMaxNb(lim:CounterLimit) : integer -> lim.maxNb]
...
[choco/minimize(pb:Problem,obj:IntVar,l:list[AbstractBranching],restart:boolean) : integer
 -> let algo := makeGlobalSearchMinimizer(obj, restart, l), 
        lims := makeDefaultLimits() in
       (attach(algo,pb), 
        for lim in lims setSearchLimit(algo,lim),
        run(algo), getBestObjectiveValue(algo))]  // v1.02: explicit access to obj. value

[choco/maximize(pb:Problem,obj:IntVar,l:list[AbstractBranching],restart:boolean) : integer
 -> let algo := makeGlobalSearchMaximizer(obj, restart, l), 
        lims := makeDefaultLimits() in
       (attach(algo,pb), 
        for lim in lims setSearchLimit(algo,lim),
        run(algo), getBestObjectiveValue(algo))]  // v1.02: explicit access to obj. value
...
[choco/makeNodeLimit(n:integer) : NodeLimit -> NodeLimit(maxNb = n, unit = "nd")]
[choco/makeBacktrackLimit(n:integer) : BacktrackLimit -> BacktrackLimit(maxNb = n, unit = "bk")]
[choco/makeTimeLimit(n:integer) : TimeLimit -> TimeLimit(maxNb = n, unit = "ms")]
[choco/makeDiscLimit(n:integer) : DiscLimit -> DiscLimit(maxNb = n, unit = "disc")]

[choco/makeDefaultLimits() : list<GlobalSearchLimit>
...
[choco/setDiscLimit(algo:GlobalSearchSolver,maxNbDiscepancies:integer) : void 
