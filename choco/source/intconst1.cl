...
[self_print(self:LinComb) : void
  -> let n := length(self.vars) in
    ...
    ; printf("~S scalar ~S",self.coefs,self.vars),
     ...
     if (self.op_cstype = EQ) princ("== 0")
     else if (self.op_cstype = GEQ) princ(">= 0")
     else princ("<> 0")]
...
[choco/assignIndices(c:LinComb, root:(CompositeConstraint U Delayer), i:integer) : integer
 -> let j := i in
       (for k in (1 .. c.nbPosVars)
           (j :+ 1, connectIntVar(root,c.vars[k],j,true,(c.op_cstype != GEQ),true,false),
        ...
        for k in (c.nbPosVars + 1 .. c.nbVars)
           (j :+ 1, connectIntVar(root,c.vars[k],j,true,true,(c.op_cstype != GEQ),false),
...
[choco/opposite(c:LinComb) : LinComb
 -> if (c.op_cstype = EQ)  makeLinComb(list{i | i in c.coefs}, c.vars,c.cste, NEQ)  // v1.011 coefs is an array
    else if (c.op_cstype = GEQ) makeLinComb(list{-(i) | i in c.coefs}, c.vars, -(c.cste + 1), GEQ)
    else (assert(c.op_cstype = NEQ),
...
choco/cfilter :: property(range = void)
...
[choco/filterOnImprovedLowerBound(c:LinComb) : boolean
 -> if (c.op_cstype = EQ) // <hha> v0.17 constraint check only for equality constraints (otherwise passive constraint)
       propagateNewLowerBound(c,computeLowerBound(c))
   else if (c.op_cstype = GEQ) false   // v1.0 (nothing to propagate => nothing was inferred, so return false)
   else (assert(c.op_cstype = NEQ),
...
[choco/filterOnImprovedUpperBound(c:LinComb) : boolean
 -> let myub := computeUpperBound(c) in
      (if (c.op_cstype = EQ) propagateNewUpperBound(c,myub)
       else if (c.op_cstype = GEQ) propagateNewUpperBound(c,myub)
       else (assert(c.op_cstype = NEQ),
...
[choco/cfilter(c:LinComb, startWithLB:boolean, minNbRules:integer) : void
...
(interface(filterOnImprovedLowerBound), interface(filterOnImprovedUpperBound), interface(cfilter))
(interface(LinComb, cfilter, filterOnImprovedLowerBound, filterOnImprovedUpperBound))

// v0.33 <thb>: this same function is used for propagating immediately or delayed
[choco/propagate(c:LinComb) : void
 -> if (c.improvedLowerBound & c.improvedUpperBound) cfilter(c, true, 2)  // apply both rules at least once, starting with LB rule
    else if c.improvedLowerBound cfilter(c, true, 1)   // apply LB rule at least once and ocsillate if this produces new inferences
    else if c.improvedUpperBound cfilter(c, false, 1)] // apply UB rule at least once and ocsillate if this produces new inferences
 ...
 -> assert(forall(v in c.vars | isInstantiated(v))),
    let s := 0 in
      (for i in (1 .. c.nbVars) s :+ (c.vars[i].value * c.coefs[i]),
       s :+ c.cste,
       if (c.op_cstype = EQ) (s = 0)
       else if (c.op_cstype = GEQ) (s >= 0)
       else (assert(c.op_cstype = NEQ), (s != 0)) )]
...
[choco/awakeOnInf(c:LinComb, idx:integer) : void
 -> if (idx <= c.nbPosVars) cfilter(c,true,1)
    else cfilter(c,false,1)]
[choco/awakeOnSup(c:LinComb, idx:integer) : void
 -> if (idx <= c.nbPosVars) cfilter(c,false,1)
    else cfilter(c,true,1)]
